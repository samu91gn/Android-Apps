...
/**
 * dialog pop-up per dettagli di una caduta:
 * mostra il grafico e altri dettagli relativi alla caduta
 */
...
public class FallDetailsDialogFragment extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		 ...
		 */
		plot = new Plot(getActivity(), new Data(0,0), 2000);
		...
		new Thread() //thread ha lo scopo di non creare interruzioni nell'UI durante l'animazione
		{
			@Override
			public void run()
			{
				...
				int timeMillis = -1000;
				float sizeLeft= 2000; //per arrivare a +1000
				int numLeft = 0;
				...
				for(int i = acquisitionList.size()-1; i >= 0 ; i--)
				{
					...
					if(numLeft > 1){
						...
						plot.pushValue(new Data(1000*1000, Math.sqrt(a.getXaxis()*a.getXaxis() + a.getYaxis()*a.getYaxis() + a.getZaxis() * a.getZaxis())));
