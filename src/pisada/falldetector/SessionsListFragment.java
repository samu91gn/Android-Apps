...
/**
 * fragment contenente la lista di sessioni
 */
...
public class SessionsListFragment extends FallDetectorFragment implements ServiceReceiver {
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		...
		if(((SessionListCardAdapter) rView.getAdapter()).itemSelectedNumber()>0){
			activity.getSupportActionBar().setTitle(((SessionListCardAdapter) rView.getAdapter()).itemSelectedNumber()+" selected");
			menu.findItem(R.id.delete_bar).setVisible(true);
			menu.findItem(R.id.archive_bar).setVisible(true);
		}
		else activity.getSupportActionBar().setTitle("SessionList");
