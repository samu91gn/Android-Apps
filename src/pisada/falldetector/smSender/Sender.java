 ...
 * che venga scambiata la card con quella nuova che contiene il valore aggiornato del campo booleano "notificainviata".
 * 
 */
...
public class Sender {
	...
	public void sendEmailToList(ArrayList<String> email, Context ctx, String message, final FallDataSource.Fall fall)
	{
		if(fds == null )
			fds = new FallDataSource(ctx);
		...
		for(int i=0;i<email.size();i++){
			contatti += email.get(i)+";";
		}
		Intent emailIntent = new Intent(Intent.ACTION_SENDTO, Uri.fromParts("mailto",contatti, null));
		emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Fall Detector");
		emailIntent.putExtra(Intent.EXTRA_TEXT, message);
		ctx.startActivity(Intent.createChooser(emailIntent, ctx.getResources().getString(R.string.sendemail)));
