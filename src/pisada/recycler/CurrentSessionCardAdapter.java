...
public class CurrentSessionCardAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements ServiceReceiver {
		...
		public FirstCardHolder(View v) {
			...
			if(sds.existCurrentSession())
			{
				if(sds.currentSession().isOnPause())
					playPause.setBackground(activity.getDrawable(R.drawable.button_selector_play));
				else
					playPause.setBackground(activity.getDrawable(R.drawable.button_selector_pause));
			}
	...
	public  class SecondCardHolder extends RecyclerView.ViewHolder {
		...
		public SecondCardHolder(View v) {
			...
			graph = new Plot(activity, new Data(timePoint, 0), null);
	...
	public CurrentSessionCardAdapter(View v, Activity activity, long time, boolean startChron, long pauseTime, boolean isPort) {
		...
		pause =activity.getDrawable(R.drawable.button_selector_pause);
		play =activity.getDrawable(R.drawable.button_selector_play);
	...
	public void onBindViewHolder(ViewHolder holder, int i) {
		...
		if(i==0 && isPortrait){ //se e' la prima o la seconda non fare niente

		}
		else if(i == 1 && isPortrait){ //(lasciati separati per facilitare edit futuri e rimarcare il distacco tra le prime due l'una dall'altra)
	...
	public SessionDataSource.Session addSession(String name, long timeStart, long timeEnd) {

		SessionDataSource.Session session = null;
		if(!sds.existSession(name)){

			try{
				session = sds.openNewSession(name, timeStart, timeEnd);
			}
	...
	private OnClickListener onPlayPauseClick = new View.OnClickListener() {
		...
		public void onClick(final View v) {
			...
			v.setBackground(activity.getDrawable(R.drawable.nonclickable));
			/*
			 * qui per due secondi setto un caricamento in mezzo. poi cambia. per quei due secondi è anche inclickabile
			 */
			...
			if(currentSessionName.equals(sessionNameDefault)) 	currentSessionName = "Session"+time;
			...
			if(!ForegroundService.isRunning()){
				...
				if(!sds.existCurrentSession()){
					...
					session = null;
					session = addSession(currentSessionName, time, 0);
